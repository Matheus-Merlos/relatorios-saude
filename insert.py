...
def search_latest_csv(filter_func: Callable[[Path | str], bool]) -> Path:
    ...
    latest_filtered_csv: Path = max(
        filter(filter_func, files), key=lambda file: file.stat().st_mtime)
    return latest_filtered_csv


def import_data(file_name_filter: str, procedure_insert: Procedures, dotenv_filename: str = '.ent', refresh_procedure: str | None = None) -> None:
    latest_file = search_latest_csv(lambda filename:
